...
- 2026-08-11: Fix truncate teks label di kartu milestone M2 (4 flags) di `ActiveMilestoneCard` & `UpcomingMilestoneCard` (SkinJournalMainView.swift): label "Results/in 8 weeks" terpotong jadi "Re.../in 8 wee..." karena HStack 4 flags + connector 3-dash (3×30pt) melebihi lebar kartu (366pt > 318pt konten). Perbaikan: font label 12/10 → 11/9 + `.lineLimit(1)` + `.minimumScaleFactor(0.85)`; connector `ThreeDashConnector` dirampingkan (8pt→6pt, spacing 3→2, hemat ~24pt). Terverifikasi: teks utuh (vision + pixel 27→36pt), posisi kartu tetap saklak (y 664-858 & 688-867 identik antar state). DEBUG hook baru `--journey-m1` & `--journey-m2` (reproduksi preview states di simulator).
...
- 2026-08-12: Update `SkinJournalCalendarView.swift` (`DayCell`): jika ada foto jurnaling pada tanggal tersebut, sel tanggal otomatis diisi penuh (`scaledToFill`) dengan foto hasil capture pengguna hari itu dengan overlay teks tanggal yang kontras, serta menghapus ikon kamera (`camera.fill`). Build sukses.
- 2026-08-12: Redesain header bulan di `SkinJournalCalendarView.swift`: menghapus container terpisah pada bagian navigasi bulan ("Agustus 2026") dan menempatkan tombol navigasi bulan langsung di bagian atas dalam kartu utama kalender mepet dengan header hari & grid tanggal. Build sukses.
- 2026-08-12: Update alur jurnaling dan layout `JourneyMainView.swift`: (1) Mengembalikan photo box (`ProgressPhotoCard` "no photos yet") di bagian atas `JourneyMainView`; (2) Tombol sky blue **"Start Journey"** di bawah container milestone diset **disabled/abu-abu** saat empty state; (3) Alur navigasi: pengguna menekan container `"add your skincare journey"` -> `ChooseProductView` -> `SelectedProductView` -> tap *"Start Journey"* -> **kamera terbuka** -> capture foto -> **langsung kembali ke `JourneyMainView`** (foto terpasang di container atas, container tengah langsung berubah dari `"add your skincare journey"` menjadi kartu milestone `ActiveMilestoneCard`, dan tombol *"Start Journey"* di bawah berubah aktif/sky blue); (4) Menekan tombol **"Start Journey"** yang aktif / kartu milestone di `JourneyMainView` membuka kuesioner (`SelfAssessmentView`) -> catatan jurnal (`JournalEntryView`) -> simpan; (5) Setelah jurnal di-save sampai tahap akhir, header *"Current Skincare Journey"* & `UpcomingMilestoneCard` muncul dan tombol **"Start Journey"** di bagian bawah disembunyikan/dihapus; (6) Progress track pada `ActiveMilestoneCard` untuk **Milestone 1** terdiri dari tepat **14 segmen dash `'-'`** (1 dash/hari dalam 2 minggu) yang warnanya berubah biru tua sesuai hari berjalan; sedangkan untuk **Milestone 2** (4 flag), konektor antar flag terdiri dari **3 segmen dash `'- - -'`** (`ThreeDashConnector`); (7) **Fix bug pembuatan journey baru**: menghapus pengecekan `if viewModel.latestJourney == nil` pada `SelectedProductView.onStartJourney` agar pembuatan journey baru selalu menginisialisasi `SkincareJourney` bersih yang selalu dimulai tepat pada **Milestone #1**. Build sukses.
- 2026-08-12: Update `SkinJournalCalendarView.swift` ("Journal Calendar"): mengubah navigasi bulan horizontal (tombol panah kanan-kiri `chevron.left`/`chevron.right`) menjadi **vertical scroll continuous** (gaya Instagram Archive calendar). Setiap bulan ditampilkan dalam kartu kalender terpisah ([MonthCalendarCard](file:///Users/ibal/Documents/C4-MySkin/C4-MySkin/Features/SkinJournal/Views/SkinJournalCalendarView.swift#L187)) lengkap dengan judul nama bulan & tahun, header hari `Min-Sab`, dan grid tanggal berfoto yang dapat di-scroll mulus ke atas/bawah. Build sukses.
- 2026-08-12: **Fix bug Milestone #2 prematur**: menghapus eksekusi `j.milestones[milestoneIndex].isCompleted = true` saat menyimpan catatan jurnal harian pertama di `JournalEntryView` (pada `SkinJournalRootView.swift`). Menyimpan catatan jurnal harian kini tidak lagi menyelesaikan seluruh Milestone #1 (2 minggu) secara prematur, sehingga perjalanan skincare yang baru dibuat tetap konsisten berada di **Milestone #1** dengan 14 segmen dash. Build sukses.
- 2026-08-12: **Preview Milestone 2 di JourneyMainView**: Menambahkan `#Preview("Milestone 2 State")` dan `#Preview("Active State (Milestone 1)")` pada [JourneyMainView.swift](file:///Users/ibal/Documents/C4-MySkin/C4-MySkin/Features/SkinJournal/Views/JourneyMainView.swift#L430-L490) dengan data sintetis `sampleMilestone2`. Pengembang dapat langsung melihat dan memverifikasi tampilan visual Milestone 2 (4 flag node, konektor 3-dash, dan `UpcomingMilestoneCard`) pada Xcode Canvas secara instant tanpa perlu menjalankan alur manual. Build sukses.
- 2026-08-13: Memperbaiki hasil merge conflict di `JourneyDetailView.swift` yang mencampur API layar Selected Product dengan layar Journal History. `JourneyDetailView` kembali menerima `SkincareJourney`, `initialIndex`, dan `hideProgressBar`; pager foto, ringkasan assessment, catatan, serta timeline dipulihkan. Route `.journeyDetail` di `SkinJournalRootView` diselaraskan dan build iOS Simulator sukses.
...
- 2026-08-13: Menambahkan end flow Milestone 1 setelah Save jurnal pada hari ke-14. Save tersebut menandai Milestone 1 selesai dan membuka `MilestoneOneCompletionView` sesuai mockup (pertanyaan lanjut Milestone 2, tombol No/Yes, mascot Lottie besar). Pilihan **Yes** kembali ke `JourneyMainView` dengan Milestone 2 aktif memakai logic yang sudah ada; pilihan **No** mengarsipkan ID journey secara persisten di `SkinJournalStore`, menghilangkan active journey dari main page tanpa menghapus jurnal/foto, dan journey tetap tersedia di History. Route detail History kini membawa `journeyID` agar archived journey yang dipilih dapat dibuka dengan benar; rentang akhir history menggunakan tanggal milestone terakhir yang benar-benar selesai. Ditambahkan DEBUG hook `--milestone-1-completion`. Build dan QA visual iPhone 17 Pro Simulator sukses.
- 2026-08-13: Menambahkan end flow Milestone 2 sesuai mockup melalui `MilestoneTwoCompletionView`. Setelah Save jurnal pada akhir periode Milestone 2 (empat periode 14 hari/8 minggu setelah Milestone 1 selesai), milestone terakhir ditandai selesai lalu layar hasil menampilkan ucapan kelulusan, rating bintang 1–5 interaktif, tombol Next, dan mascot. Next menyimpan `experienceRating`, mengarsipkan journey agar tidak lagi aktif, dan mempertahankan seluruh data serta rating di History. Model `SkincareJourney` mendapat field opsional `experienceRating` agar data lama tetap kompatibel. Ditambahkan DEBUG hook `--milestone-2-completion`; build dan QA visual iPhone 17 Pro Simulator sukses.
- 2026-08-13: Menambahkan konfigurasi demo melalui resource `Resources/demo.plist` dengan boolean `isDemoEnabled`. Saat `true`, Main Page menampilkan dua tombol berbeda: **Skip to M1** (biru) menyiapkan journey di akhir Milestone 1 dan membuka `MilestoneOneCompletionView`; memilih Yes melanjutkan ke `JourneyMainView` dalam state Milestone 2. **Skip to M2** (kuning) menyiapkan kedua milestone selesai dan membuka `MilestoneTwoCompletionView`. Jika plist `false` atau tidak terbaca, tombol tidak dibuat dan kedua action dilindungi guard. Loader baru `DemoConfiguration` membaca property list dari app bundle dengan default aman `false`. `demo.plist` tervalidasi, masuk app bundle, build sukses, dan layout kedua tombol diverifikasi pada iPhone 17 Pro Simulator.
- 2026-08-13: Memindahkan kontrol demo milestone dari `SkinJournalMainView` ke kanan atas `JourneyDetailView`. Tombol diperkecil menjadi capsule **To M1** dan **To M2**, tetap hanya tampil jika `demo.plist` mengaktifkan `isDemoEnabled`, serta tetap memakai logic skip milestone yang sama dari root navigation.
- 2026-08-13: Mengadaptasi visual area face alignment dari repo `TestFacialLandmarkDetection` ke `CameraGuideView`: area di luar target diberi mask gelap, outline wajah berubah kuning/hijau sesuai status, dan ditambahkan garis bantu silang putus-putus. Mekanik lama (`VNDetectFaceRectanglesRequest`, threshold, hysteresis, capture/crop, dan confirmation flow) tetap dipertahankan; tombol Back tetap tersedia.
- 2026-08-13: Speech bubble mascot di `SkinJournalMainView` kini memilih acak dari 10 skincare tips berbahasa Inggris dan berganti setiap 5 detik dengan transisi opacity tanpa mengulang tip yang sama secara berurutan. `SpeechBubbleView` mendapat parameter opsional `maxWidth` agar kalimat panjang membungkus rapi tanpa mengubah penggunaan lama di Self Assessment.
